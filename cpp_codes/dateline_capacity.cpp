...
- az eliminate_cutting_srlg=true flagnél nem történik semmi
*/
...
int SRLG_Capacities[Max_Dual_Node_Count];
...
bool Edge_Disjoint_Paths=false; // ezt is ki kell venni, hogy legyen valtozas
bool Bigger_SRLG_Capacity=true; // legyen-e nagyobb SRLG kapacitas (most csak 0 esetben vannak tesztek)
bool eliminate_cutting_srlg=false; // ha Bigger_SRLG_Capacity=0 akkor erdemes 1-re beallitani
...
int Dual_Node_Count; // = Primal_Area_Count + number of SRLGs
// format: destination, weight, edge_id
vector<weighted_edge> Dual_Adj_List[Max_Dual_Node_Count];
int SRLG_Count;
vector<vector<int>> SRLGs = {{}}; // SRLGs[i] stores the edge ids of the i-th SRLG
 ...
 * - `eliminate_cutting_srlg`: Boolean flag to determine if cutting SRLGs should be removed.
 ...
 */
void add_SRLG(vector<int> edges, int SRLG_Capacity, int srlg_id) {
    ...
    while (q.size()>0) {
        ...
        for (auto x:Area_Adj_List[Cur_Area]) {
            ...
            if (!Is_Visited[Next_Area]) {
                ...
                q.push(Next_Area);
            } else { // if the area is already visited, check if the distance is correct
                if (Dist[Next_Area] != Next_Dist) {
                    cutting_srlg = true; // this is a cutting srlg (because it goes around s or t)
                }
            }
        }
    }
    ...
    /*if (cutting_srlg) { // check if the SRLG is cutting
        if (!eliminate_cutting_srlg) { // if we don't want to eliminate cutting srlgs, we can include it to the upper bound

            Answer_Upper_Bound=min(Answer_Upper_Bound, SRLG_Capacity);
        }
    } else {*/ // add slrg to the dual graph with an extra node
        ...
        SRLG_Capacities[Dual_Node_Count]=SRLG_Capacity;
    //}
}
 ...
 * It also reads the SRLG capacity if the flag `Bigger_SRLG_Capacity` is set.
 ...
 *         - Optionally, the SRLG capacity if `Bigger_SRLG_Capacity` is set.
 * 
...
void read_SRLGs() { 
    ...
    for (int i=1; i<=SRLG_Count; i++) { 
        ...
        for (int j=1; j<=SRLG_Size; j++) { 
            ...
            if (Edge_id==0) {
                cout << "Wrong input: there is a problem with SRLG " << j << endl;
                exit(0);
            }
            edges.push_back(Edge_id);
        }

        // read the SRLG capacity if the flag is set
        if (Bigger_SRLG_Capacity) {
            cin >> SRLG_Capacity;
        }


        // add the SRLG to the list
        SRLGs.push_back(edges);
        add_SRLG(edges, SRLG_Capacity, i);
    }
}
...
void find_negative_cycle_srlgs(){
    ...
    for (int srlg_id: negative_cycle) {
        negative_cycle_srlgs.push_back(SRLGs[srlg_id]);
        for (int x: SRLGs[srlg_id])
    ...
    for (int Cur_Node=1; Cur_Node<=Dual_Node_Count; Cur_Node++) {
        for (auto x:Dual_Adj_List[Cur_Node]) {
            ...
            if (bigger_node<=Primal_Area_Count) { // if both nodes are primal areas
                additional_weight=(Edge_Disjoint_Paths ? 1 : Val); // ez normál élnél a keresett utak száma lesz
            }
...
int main()
{
    ...
    // binary search for the number of paths
    int lo=0, hi=min(Answer_Upper_Bound, SRLG_Sum)+10, mid;
    while (hi-lo>1) {
        mid=(hi+lo)/2;
    ...
    if (lo>inf) {
        cout << "Infinitely many paths" << endl;
    } else if (lo>1000000) {
        cout << "Number of paths: " << lo << endl;
    } else { // we know how many path we are looking for
        //find_srlg_disjoint_walks(lo, true);
        find_srlg_disjoint_path(lo, true);
        print_the_answer();
        print_potential_and_neighbors();
        find_srlg_disjoint_path(lo+1, true, true);
        find_negative_cycle_srlgs();
    }
...
*/
/*
3-1-es pelda, nem kene kitorolni
12
0 2 1
1 0 2
1 1 3
